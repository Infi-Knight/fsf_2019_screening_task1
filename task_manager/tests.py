...
class TaskModelTests(TestCase):
    ...
    def test_has_duedate_passed_with_due_on_passed(self):
        """
        is_overdated() returns True for task whose due_on
        is passe
        """
        date = datetime.date.today()
        overdated_task = Task(
            title='Task1', due_on=date-datetime.timedelta(days=1))
        self.assertIs(overdated_task.has_duedate_passed(), True)

    def test_has_duedate_passed_with_date_not_passed(self):
        """
        has_duedate_passed() returns Flse for task whose
        due_on is not passed
        """
        date = datetime.date.today()
        due_task = Task(title='Task2', due_on=date +
                        datetime.timedelta(days=1))
        self.assertIs(due_task.has_duedate_passed(), False)
...
class ViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.task1 = Task.objects.create(
            title='task1', creator=cls.u1, due_on=datetime.date.today())
        cls.task1.assigned_to.add(cls.u1)
        cls.task2 = Task.objects.create(
            title='task2', creator=cls.u2, due_on=datetime.date.today())
    ...
    def test_view_team_detail_with_no_permission(self):
        ...
        url = reverse('task_manager:team_detail',
                      kwargs={'team_id': self.team1.id})
    ...
    def test_view_team_add_member_without_permission(self):
        ...
        """
        self.client.force_login(self.u3)
        url = reverse('task_manager:team_add_member',
                      kwargs={'team_id': self.team2.id})
    ...
    def test_view_team_remove_member_with_permission(self):
        ...
        url = reverse('task_manager:team_remove_member',
    ...
    def test_view_task_create(self):
        ...
        response = self.client.post(
            url, {'title': 'task', 'due_on': datetime.date.today()})
        task = Task.objects.get(title='task')
        redirect_url = reverse('task_manager:task_detail',
    ...
    def test_view_task_edit_without_permission(self):
        ...
        """
        self.client.force_login(self.u1)
        url = reverse('task_manager:task_edit', kwargs={
    ...
    def test_view_task_delete_without_permission(self):
        ...
        """
        self.client.force_login(self.u1)
        url = reverse('task_manager:task_delete',
    ...
    def test_view_task_delete_with_permission(self):
        ...
        task = Task.objects.create(
            title='task', creator=self.u1, due_on=datetime.date.today())
    ...
    def test_view_task_detail_without_permission(self):
        ...
        """
        self.client.force_login(self.u3)
        url = reverse('task_manager:task_detail',
                      kwargs={'task_id': self.task2.id})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 403)
    ...
    def test_view_task_comment_without_permission(self):
        ...
        """
        self.client.force_login(self.u3)
        url = reverse('task_manager:task_comment',
                      kwargs={'task_id': self.task2.id})
