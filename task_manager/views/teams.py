...
def team_create(request):
    ...
    if request.method == 'POST':
        if request.POST.get('team_name'):
            ...
            messages.success(
                request, "New team `{0}` has created successfully!".format(team))
    ...
    if request.user == team.leader:
        try:
            ...
            if user not in team.members.all():
                ...
                messages.success(
                    request, "The user `{0}` has added to your team successfully!".format(user))
            else:
                messages.error(
                    request, "The user `{0}` is already in team".format(user))
        except:
            messages.error(request, "User with username `{0}` is not found".format(
        ...
        try:
            ...
            if request.user == team.leader:
                if user != team.leader:
                    ...
                    messages.success(
                        request, "The user `{0}` has been removed from team".format(user))
                elif user == team.leader:
                    messages.error(
                        request, "Leader can not be removed from team!")
        except:
            messages.error(request, "User with username `{0}` is not found in your team!".format(
