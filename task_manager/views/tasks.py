...
def task_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Task: `{0}` is created successfully!".format(new_task.title))
        ...
        if request.method == 'POST':
            form = TaskEditForm(
                ...
                initial={'members': members,
                         'has_team': has_team, 'teams': teams}
            )
            if form.is_valid():
                form.save()
                messages.success(
                    request, "The changes are saved successfully!")
...
def task_accept(request, task_id):
    ...
    if request.user in task.assigned_to.all():
        if task.status == 'PLAN':
            task.status = 'PROG'
            task.task_accepted_on = datetime.date.today()
            task.task_accepted_by = request.user
            task.save()
            messages.success(
                request, "The task has marked as In-Progress successfully!")
...
def task_mark_completed(request, task_id):
    ...
    if request.user == task.creator or request.user in task.assigned_to.all():
        if task.status == 'PROG':
            ...
            task.save()
            messages.success(
                request, "The task is marked as Completed successfully!")
