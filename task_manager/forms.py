...
class TaskCreationForm(ModelForm):
    ...
    desc = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={"class": "w3-input",
                   "placeholder": "Describe about task", "rows": 5}
        ),
        required=False
    )
...
class TaskEditForm(ModelForm):
    ...
    class Meta:
        model = Task
        fields = ('title', 'desc', 'due_on', 'team', 'assigned_to')
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get('initial').get('has_team'):
            ...
            self.fields["assigned_to"].queryset = kwargs.get(
                'initial').get('members')
        else:
...
class SignUpForm(forms.ModelForm):
    ...
    password2 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={"class": "w3-input",
                   "placeholder": "Enter same Password, for verification"}
        ),
        strip=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["username"].widget.attrs.update(
            {"autofocus": True, "class": "w3-input",
                "placeholder": "Create a unique username for you!"}
        )
        self.fields["email"].widget.attrs.update(
            {"class": "w3-input", "required": True,
